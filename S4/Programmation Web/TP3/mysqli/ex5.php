...
if (isset($_GET['nom']) && isset($_GET['prenom'])) {
	...
	$requete='SELECT v.immat, m.modele, v.couleur, v.datevoiture FROM voiture v, cartegrise cg, proprietaire p, modele m WHERE v.immat = cg.immat AND cg.id_pers = p.id_pers AND v.id_modele=m.id_modele AND UPPER(p.nom) = "'.$nom.'" AND UPPER(p.prenom) = "'.$prenom.'" ORDER BY v.immat';	
	$result=@mysqli_query($idcom,$requete);
	...
	if(mysqli_num_rows($result) == 0) {
		...
		while($test = mysqli_fetch_field($result)) echo '<th>'.$test->name.'</th>';
